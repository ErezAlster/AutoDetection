...
class ObjectDetectionUtils:
    ...
    def extract_detections(self, input_data: list, range) -> dict:
        """
        ...
        for i, detection in enumerate(input_data):
            ...
            for det in detection:
                ...
                absolutepixels = [round(bbox[1] * width) + offestX, round(bbox[0] * height) + offestY, round(bbox[3]*width) + offestX, round(bbox[2]*height) + offestY]
                results.append({
                    "confidence": float(score),
                    "type": "rectangle",
                    "label": self.labels[i],
                    "points": absolutepixels,
